...
public static class ChartRepository
{
  ...
  {
    var loadedPer = await loadedPeriodRep.TryGetByAsync(instrumentId, timeFrameId, cancellationToken);
    if (loadedPer == null)
      throw new ArgumentException("No data for InstrumentId and TimeFrameId");
    return loadedPer;
  }

  public static async Task<Chart?> TryGetByAsync(this IReadRepository<Chart> loadedPeriodRep, int instrumentId, int timeFrameId, CancellationToken cancellationToken = default)
  {
